...
/* 페이징 */	
	.paging {text-align:center; width:870px; position:absolute; bottom:10px}
	.pageNo{color:#f36359;}
</style>
</head>
<%
...
olist.add(o1);
olist.add(o2);
ArrayList<Orderlist> o_list = (ArrayList<Orderlist>)session.getAttribute("olist");
...
<!-- 주문/배송조회 -->
	...
	<div class="paging">
			...
			if(pg.isPre()){
			%>
				<a href="orderlist.jsp?i_page=<%=preNo %>">Pre</a>
			<%
			}
			for(int i = pg.getPage_Start(); i <= pg.getPage_End();i++){
				if(i == i_page){
			%>
				<a class="pageNo" href="orderlist.jsp?i_page=<%=i %>" ><%=i %></a>
			<%	}else{ %>
				<a href="orderlist.jsp?i_page=<%=i %>"><%=i %></a>
			<%	}
			}
			if(pg.isNext()){
			%>
				<a href="orderlist.jsp?i_page=<%=nextNo %>">Next</a>
			<%} %>
			</h4>	
		</div>
	</section>
