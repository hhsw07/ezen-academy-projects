...
2. dispatcher-servlet.xml에 Controller 클래스 등록.
	- <bean class="springweb.a01_basic.A03_MemberCtrl"/>
3. 요청 url 정리
	- http://localhost:5080/springweb/login.do
4. 메서드 선언	
	- 요청 url의 pattern에 맞는  @RequestMapping("login.do") 선언
	- 연결되는 메서드 선언
		public String login(){
			return "";
		}
	- 해당 url을 통해서 호출되는 jsp 화면단 
		파일 생성과 return 값으로 정의 
		springweb\WebContent\WEB-INF\views\a01_login.jsp
		파일 생성.
		return "springweb\WebContent\WEB-INF\views\a01_login.jsp";
	- 요청값 매개변수 선언
	- Model 데이터 선언
	- Model 데이터 할당.
5. 화면구현.a01_login.jsp
