...
/*
# 요청값을 객체로 처리하기..
1. 스프링에서는 요청값을 useBean과 같이  setXXX로
	요청값을 받아서 처리할 수 있다.
	?name=홍길동&age=25&loc=서울신림동
	요청을 받은 매개변수로 VO객체를 선언하고,
	Vo객체에 setName(), setAge(), setLoc()가 있으면,
	요청값으로 해당 객체의 field(속성)에 할당할 수 있다.
 * */
	...
	/*	
# 요청값과 모델 화면단 처리의 분리..
1. 스프링의 요청값 key=val&key=val
	1) @RequestParam("요청key") String val
	2) 요청값을 처리해주는 객체 : 
		Person p : 요청 key와 mapping되는 methods가 있을 때
