...
<script type="text/javascript">
	$(document).ready(function(){
		...
		<%--
		// 자바스크립트에 초기(화면이 로딩될시)에 server단에서 넘겨온
		// 변수를 할당 처리한다.(서버단에서 java프로그램을 결과를 만들어서)
		// 넘겨준다. server에서 ${model01}를 통해서 실제 모델 데이터를
		// 만들어 결과물만 넘겨주기 때문에 client단 코드에서는 el등 java관련
		// 된 코드를 볼 수 없다..
		--%>
		var model = "${model01}";
		alert(model);
		...
		*/
		...
		$("#ajaxBtn").click(function(){
			...
			xhr.open("get","${path}/callServer.do?name=홍길동",true);
		...
		$(".sch").keyup(function(){
			...
			xhr.open("get","${path}/emplist.do?"+param,true);
			xhr.onreadystatechange=function(){
				if(xhr.readyState==4&&xhr.status==200){
