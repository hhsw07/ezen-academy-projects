...
jsp는 변수들을 저장할 때, 특정한 범위로 저장을 할 수 있다..
	이때, 
	활용되는 객체가 scope범위로 한 페이지 단위에서 데이터를 저장하고 활용할 수 있는 pageContext, 
	요청 page까지 데이터를 저장하고 활용할 수 있는 request, 
	브라우저가 닫힐 때까지 데이터를 저장하고 활용할 수 있는 session,
	서버단위로	데이터를 저장하고 활용할 수 있는 application으로 기본 객체를
	지원하고 있다.
...
int member_point = 25;
session.setAttribute("아이디",member_id);
session.setAttribute("이름",member_name);
session.setAttribute("포인트",member_point);
%>
...
3. 웹서버에서 지정한 file 서버의 물리적 위치를 지정하여, 해당 서버를 통해 처리하고자 한다.
  ...
  <context-param>
	...
	<param-value>C:\a01_javaexp\workspace_web\jspexp\WebContent\z01_fileLoc</param-value>  
  </context-param> 
