...
public class A02_EmpService {
	...
	// 수정 처리
	public void updateEmp(HttpServletRequest req) {
		// 1. 수정할 요청값.
		int empno= toInt(req.getParameter("empno"));
		String ename = toStr(req.getParameter("ename"));
		String job = toStr(req.getParameter("job"));
		int mgr = toInt(req.getParameter("mgr"));
		double sal = toDbl(req.getParameter("sal"));
		...
		// 2. VO에 요청값 할당.
		// 3. 수정 dao 호출 처리..
		dao.updateEmp(new Emp(empno, ename, job, mgr,null,sal,comm, deptno));
	}	
	// 삭제처리 
	public void deleteEmp(HttpServletRequest req) {
		dao.deleteEmp(toInt(req.getParameter("empno")));
	}	
